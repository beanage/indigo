...
public:
    ...
    void upload() override
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, num_verts_ * sizeof(vertex), data.data(),
                     GL_STATIC_DRAW);

        glEnableVertexAttribArray(vertex_attribute_index);
        glVertexAttribPointer(vertex_attribute_index, 3, GL_FLOAT, GL_FALSE,
        ...
        glVertexAttribPointer(
            texcoord_attribute_index, 2, GL_FLOAT, GL_TRUE, sizeof(vertex),
...
static std::tuple<int, int, int> parse_face_indices(const std::string& input)
{
    ...
    return std::make_tuple(parse_face_indices_number(parts[0]),
        ...
        if (type == "v") {
            ...
            for (int i = 0; i < vertex_count; ++i) {
                std::tuple<int, int, int> indices =
                    parse_face_indices(vert[vertex_order[i]]);
