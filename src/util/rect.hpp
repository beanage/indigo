...
{
    return out << "{{" << r.left << ", " << r.top << "}, {" << r.right << ", "
               << r.bottom << "}}";
}

template <typename T>
basic_rect<T> centered_rect(typename basic_rect<T>::type bounds_w,
                            typename basic_rect<T>::type bounds_h,
                            const basic_rect<T>& rect)
{
    typename basic_rect<T>::type w_diff = bounds_w - rect.width(),
                                 h_diff = bounds_h - rect.height();
    typename basic_rect<T>::type w_off = w_diff / static_cast<T>(2),
                                 h_off = h_diff / static_cast<T>(2);
