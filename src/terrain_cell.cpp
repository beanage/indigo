...
void terrain_cell::upload()
{
    ...
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_TRUE, sizeof(vertex), (void*)(sizeof(glm::vec3)*2));

    glBindVertexArray(0);
}

bool terrain_cell::intersects(glm::vec3 from, glm::vec3 to, glm::mat4 model, glm::vec3& position, unsigned& x, unsigned& y)
{
    ...
    for (unsigned i = 0; i < indices_.size(); i += 3) {
        ...
        if (glm::intersectRayTriangle(from, to, glm::vec3(glm::vec4(triangle[0], 1) * model), glm::vec3(glm::vec4(triangle[1], 1) * model), glm::vec3(glm::vec4(triangle[2], 1) * model), position)) {
...
void terrain_cell::update_height(bitmap const* heightmap)
{
    const unsigned cols = size + 1;
    const unsigned rows = size + 1;
    ...
    for (unsigned y = 0; y < rows; ++y) {
        for (unsigned x = 0; x < cols; ++x) {
            ...
            const glm::ivec2 heightmap_uv(
                x_ * size + x,
                y_ * size + y);

            if (heightmap) {
                auto pixel = heightmap->pixel(heightmap_uv.x, heightmap_uv.y);
                vert.pos.z = (pixel.r() / 255.f) - (pixel.g() / 255.f); // Das ist komisch!
            } else {
                vert.pos.z = 0.f;
            }
        }
...
void terrain_cell::update_tiles(bitmap const* opaquemap)
{
    indices_.clear();
    ...
    for (unsigned y = 0; y < size; ++y) {
        for (unsigned x = 0; x < size; ++x) {
            if (opaquemap) {
                const bool skip_tile = opaquemap->pixel(x_ * size + x, y_ * size + y).r() > 0;
                if (skip_tile)
                    continue;
            }
...
void terrain_cell::compute_normals()
{
    ...
    for (unsigned y = 0; y < rows; ++y) {
        ...
        for (unsigned x = 0; x < cols; ++x) {
            ...
            vertex const& left_vert = x > 0     ? vertices_[row_offset + x - 1]
                                    : left_cell ? left_cell->vertices_[row_offset + size - 1]
                                                : vertices_[row_offset + x];

            vertex const& right_vert = x < size   ? vertices_[row_offset + x + 1]
                                     : right_cell ? right_cell->vertices_[row_offset]
                                                  : vertices_[row_offset + x];

            vertex const& top_vert = y > 0    ? vertices_[row_offset - size + x]
                                   : top_cell ? top_cell->vertices_[size * (size - 1) + x]
                                              : vertices_[row_offset + x];

            vertex const& bottom_vert = y < size    ? vertices_[row_offset + size + x]
