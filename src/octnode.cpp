    ...
    if(is_leaf()) {
        ...
        do
        {
            ...
            if(isect_it != inner_isects.end()) {
                ...
                entry = isect.pos;
                current_node = octant_for_indices(current_node->indices + isect.step).get();
                ++isect_it;
            }
            else
...
root_octnode::root_octnode(const aabb &size) :
    ...
    top(geom.front_bottom_left + glm::vec3(0,geom.size.y,0), y_axis),
...
const entity *root_octnode::pick(const ray &r) const
{
    std::array<octnode_plane_isect, 6> bounding_isects = {
        ...
        octnode_plane_isect(r, bottom),
        octnode_plane_isect(r, left),
        octnode_plane_isect(r, right),
        octnode_plane_isect(r, front),
        octnode_plane_isect(r, back)
    };
